...
def prepare_eval_input(
    ...
    run_length = sub_examples[idx]["run_length"]
    input = np.array(
        [
            1.0,
            scores[idx],
...
def make_guess_dicts_from_question(
    question: qbdata.Question, guesser, run_length: int = 200, num_guesses: int = 5
):
...
def write_guess_json(
    ...
    with open(filename, "w") as outfile:
        for ques in tqdm(questions):
            guesses = make_guess_dicts_from_question(
                ques, guesser, run_length, num_guesses
            )
